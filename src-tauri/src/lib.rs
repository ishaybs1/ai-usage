...
struct DayRow { date: String, claude: f64, cursor: f64, total: f64 }

#[derive(Serialize)]
#[serde(rename_all = "camelCase")]
struct ModelRow { model: String, claude: f64, cursor: f64, total: f64 }
...
fn scan_usage(range: String) -> UsageSnapshot {
    ...
    let mut claude_month = 0.0; let mut cursor_month = 0.0;
    ...
    for i in 0..=(now-start).num_days() {
        ...
        days.push(DayRow{date:key,claude:c,cursor:u,total:pricing::round_cents(c+u)});
        if let Some(a)=a {
    ...
    for (key,a) in &data { if key.starts_with(&month_prefix) { claude_month += a.claude_cost; cursor_month += a.cursor_cost; } }
    let model_breakdown = models.into_iter().map(|(model,(claude,cursor))| ModelRow{model,claude:pricing::round_cents(claude),cursor:pricing::round_cents(cursor),total:pricing::round_cents(claude+cursor)}).collect();
    let yesterday = data.get(&yesterday_key).map(|a| a.claude_cost+a.cursor_cost).unwrap_or(0.0);
    UsageSnapshot {
        as_of_date: today_key, today: pricing::round_cents(today.claude_cost+today.cursor_cost), month: pricing::round_cents(claude_month+cursor_month), yesterday: pricing::round_cents(yesterday),
        claude_today:today.claude_cost,cursor_today:today.cursor_cost,claude_month:pricing::round_cents(claude_month),cursor_month:pricing::round_cents(cursor_month),sessions_today:today.sessions.len(),messages_today:today.claude_messages+today.cursor_messages,
        product_breakdown:today.claude_by_product.clone(),model_breakdown,days,top_sessions:scanner::top_sessions(today,3),tips:coach::tips(),insight:coach::insight(&today.claude_by_product,&today.claude_by_model,today.claude_cost),
        claude_path:scanner::claude_root().display().to_string(),cursor_path:scanner::cursor_db().display().to_string()
    }
}
